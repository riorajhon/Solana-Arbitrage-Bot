...
const METEORA_V2_POOL = {
  ...
  referralTokenAccount: new PublicKey('8S8HjmPZr8tNNEmMj5pcqS5RN73uF6DmcUDEDaoUQ1Ei'), // Use USDC vault as placeholder
...
async function createALT(payer: Keypair): Promise<PublicKey> {
  ...
  const addressesToAdd: PublicKey[] = [
    ...
    METEORA_V2_POOL.referralTokenAccount, METEORA_V2_POOL.eventAuthority,
...
async function testRoundTrip(
  ...
  try {
    ...
    if (result.value.err) {
      ...
      const swapsCompleted = logs.filter(log => log.includes('completed successfully') || log.includes('DAMM V2: Swap completed')).length >= 2;
