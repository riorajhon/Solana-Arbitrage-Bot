...
export interface HopData {
  ...
  poolAccountCount: number;   // Number of pool accounts for this hop (includes variable bin/tick arrays)
...
export function buildArbitrageInstructionData(route: ArbitrageRoute): Buffer {
  ...
  // Calculate size: 1 (tag) + 8 (initial) + 8 (min_output) + 1 (count) + (7 * hops)
  // Each hop: dex_program(1) + in_token_is_2022(1) + out_token_is_2022(1) + is_base_swap(1) + pool_index(1) + pool_account_count(1) + dex_program_index(1) = 7 bytes
  const dataSize = 1 + 8 + 8 + 1 + (hopCount * 7);
  ...
  for (const hop of route.hops) {
    ...
    // Pool account count (u8)
    data.writeUInt8(hop.poolAccountCount, offset);
    offset += 1;
...
export function parseArbitrageInstructionData(data: Buffer): ArbitrageRoute {
  ...
  for (let i = 0; i < hopCount; i++) {
    ...
    const poolAccountCount = data.readUInt8(offset);
    offset += 1;
...
export function getPoolAccountCount(dexProgram: DexProgram): number {
  switch (dexProgram) {
    case DexProgram.PumpAmm: return 11; // pool, global_config, vaults, fees, event_authority, creator vault, fee_config, fee_program
    case DexProgram.OrcaWhirlpool: return 7;
    case DexProgram.MeteoraDammV1: return 8;
    case DexProgram.MeteoraDammV2: return 8;
    case DexProgram.MeteoraDlmm: return 11;
...
export function buildArbitrageAccounts(
  ...
  for (let i = 0; i < hopPoolAccounts.length; i++) {
    ...
    hops.push({
      ...
      poolAccountCount: hop.accounts.length,  // Actual count of pool accounts for this hop
