#!/usr/bin/env ts-node
/**
 * Orca Whirlpool Bidirectional Test
 * Tests: 2-hop round trip SOL → USDC → SOL using same pool in both directions
 * This verifies that is_base_swap works correctly for a_to_b (true) and b_to_a (false)
 */
...
// Orca Whirlpool SOL/USDC pool (token_a=SOL, token_b=USDC)
const ORCA_POOL = {
  ...
  tickArray0: new PublicKey('8fV1MQYaRiW9qDWinzYfNa3FLs8PXSdPxoytiwrcyJPD'),
  tickArray1: new PublicKey('FdtvWk8j5u1a64YK2Uxk9eXxKZJTwLHDGx8aJPbJyw2Q'),
  tickArray2: new PublicKey('6hA1LN1fzCiXqymDiQXeBFn5da1b7STP1L7JmDc6hR3M'),
...
async function testRoundTrip(
  ...
  console.log(`   Hop 1: SOL → USDC (is_base_swap=true, a_to_b)`);
  console.log(`   Hop 2: USDC → SOL (is_base_swap=false, b_to_a)`);
  ...
  try {
    ...
    if (result.value.err) {
      ...
      // Check if both swaps completed
      const hop1Success = logs.some(log => log.includes('Hop 1') || (log.includes('Orca') && log.includes('successfully')));
      const hop2Success = logs.some(log => log.includes('Hop 2'));
...
async function main() {
  console.log('═══════════════════════════════════════════════════════════');
  console.log('🧪 Orca Whirlpool Bidirectional Test');
  console.log('   Pool: SOL/USDC (token_a=SOL, token_b=USDC)');
